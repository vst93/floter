...
pub struct ExtensionListItem {
    ...
    /// Where a manual run sends its output, read from the manifest. A manifest
    /// without the field parses as `background`, so every row written before
    /// R9-2 keeps its old behavior. The row's inline switch edits this through
    /// the ordinary update transaction.
    pub output: crate::extensions::manifest::OutputMode,
    /// R9-2 slice 3 · the declared inputs the run-time form renders, read from
    /// the same manifest `output` is. The run command re-reads `manifest.params`
    /// as its own authority, so this is a projection for rendering only — it
    /// never becomes the argv source. Empty for an integration with no
    /// parameters and for every detected (not-yet-connected) row.
    pub params: Vec<crate::extensions::manifest::ParamDefinition>,
...
impl ExtensionListItem {
    fn installed(
        ...
        // A manifest that cannot be read has no declared mode; the default
        // (background) is also what an unreadable manifest would have run as.
        let output = manifest
            .as_ref()
            .map(|manifest| manifest.output)
            .unwrap_or_default();
        // The run-time form renders these; the run command re-reads the
        // manifest as its own authority, so this projection can never be the
        // argv source.
        let params = manifest
            .as_ref()
            .map(|manifest| manifest.params.clone())
            .unwrap_or_default();
...
/// argv item; nothing is ever joined into a shell string. An absent `values` is
/// an empty map, so an integration without parameters runs exactly as before.
#[tauri::command]
pub async fn extensions_run(
    state: State<'_, ExtensionState>,
    id: String,
