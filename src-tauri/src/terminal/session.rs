...
impl TerminalSession {
    ...
    ///
    /// The directory is carried over; the history is not. Both shells keep the
    /// running session's history in memory and write it out only on exit, so the
    /// new window comes up with nothing behind the Up arrow unless the user's own
    /// configuration shares it — `setopt share_history` or `inc_append_history`
    /// in zsh, `history -a` from `PROMPT_COMMAND` in bash — in which case it is
    /// already on disk and the new shell reads it at startup for free.
    ///
    /// Asking this session's shell to flush was tried and reverted: there is no
    /// way to run a builtin in a live interactive shell without typing it at the
    /// prompt, so the command echoed into the terminal the user was working in,
    /// appended itself to whatever they had half-typed there, and still did not
    /// reliably produce a history the new window could read.
