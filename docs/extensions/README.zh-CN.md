...
表单默认生成 `custom-tool`、`1.0.0` 和空的默认执行参数；集成 ID 由后端在创建时随机分配（形如 `local.<短随机码>`），前端不参与生成。每个参数按独立 argv 项编辑，不做 Shell 字符串拆分。创建时 Floter 会生成一份静态命令描述，并验证当前运行时可用；普通可执行文件和脚本不需要实现 Provider Protocol。要在当前设备完成验证，平台列表必须包含当前平台。
...
由可视化创建器生成的单命令集成可以在集成详情或更多菜单中再次编辑。保存时 Floter 会重新验证清单、运行时和命令描述；验证失败会恢复原有文件与连接状态。集成 ID 是创建时分配一次的持久身份，与名称、命令 ID 等任何可编辑字段无关，创建后不能在编辑器中修改。
...
### 从链接一步接入（`floter://register`）

已经装在 `PATH` 上的工具不需要写 manifest：`floter://register?cmd=rg`（或终端里的
`floter register rg`）会把该命令解析成一个发现候选，并把它落到集成页的「检测到」
列表上高亮。链接本身**永不安装、永不绑定**：`cmd` 只能是单个命令名（不允许路径、
`..`、shell 元字符，长度 ≤ 64），可选的 `args` 只作为提示展示、宿主从不执行；
真正的接入仍然需要用户点击该行的接入按钮，走与 R8-2 相同的一次确认路径。
名字在设备上找不到时，「检测到」区域会就地说明，不静默失败。
