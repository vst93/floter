...
1. **elevation 阶梯是唯一阴影来源**。`--elev-0`（平面上控件：inset edge+rim）、`--elev-1`（按平，none）、`--elev-2`（浮起 pane，`--glass-raised-shadow` 的基座）、`--elev-3`（浮层：drawer/dialog/toast/menu）四档；派生档 `--elev-hover(-soft)`、`--elev-3-edge/-compact`、`--elev-4`（拖拽中的 pinned card）、`--elev-bar`、`--elev-track`、`--elev-keycap`、`--elev-halo` 都写成 `var(--elev-*)`/已有 token 的组合。宿主表里不得再出现阴影字面量：tint 必须是 token，带 cast 的层必须点名 rung（`tests/hig-craft.test.ts` 的 wiring 断言）。唯二例外：`.platform-*` 的窗口服务器边框阴影（另一套 shadow authority，已 token 化）、以及 `var(--glass-field-shadow)` 这类纯 token 别名（在定义处审计）。
