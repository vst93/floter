...
import { PermissionTierList } from "./PermissionTierList";
...
export function LocalInstallDialog({ pending, busy, t, onCancel, onConfirm, stopPropagation, dialogRef }: LocalProps) {
  return <div className="extension-permission-backdrop" role="presentation" onMouseDown={onCancel}><section ref={dialogRef} className="extension-permission-dialog" role="dialog" aria-modal="true" aria-labelledby="extension-local-title" tabIndex={-1} onMouseDown={stopPropagation}>
    ...
    <div className="extension-permission-list extension-permission-list--tiered"><div><strong>{t("settings.extensions.localSource")}</strong><span>{pending.source === "local" ? t("settings.extensions.localDirectory") : pending.source}</span></div><div><strong>{t("settings.extensions.localRuntime")}</strong><span>{pending.runtime}</span></div><div><strong>{t("settings.extensions.customPlatforms")}</strong><span>{pending.platforms.join(", ") || t("settings.extensions.unavailable")}</span></div><PermissionTierList permissions={pending.review.permissions} t={t} /><p className="extension-permission-dialog__boundary">{t("settings.extensions.permissionReviewBoundary")}</p></div>
