...
A post-implementation review identified 3 defects in the slice 3 work. All fixes applied to the working tree without reverting the round.

### Defect 1: Restore-target heuristic `cleanup_paths[0]` is wrong
**File:line**: `transaction.rs:242-267`

**Issue**: Original code used `cleanup_paths.is_empty()` to distinguish normal uninstall from custom-integration edit, but normal uninstall with `remove_data=true` also has non-empty `cleanup_paths` (contains data root, see `install.rs:1347-1354`). For that journal, a crash before lock commit would rename the staged extensions-dir directory into the data root—wrong location, and a regression vs the previous behavior.

**Fix**: Decide restore target from WHERE `staged_path` lives, not from `cleanup_paths`:
- `staged_path.starts_with(state.paths.extensions)` (canonical path comparison) → normal extension uninstall, restore to `state.paths.extensions.join(&journal.extension_id)`
- Otherwise (custom-integration edit backup inside data/) → restore to `cleanup_paths[0]`
- `staged_path = None` → nothing to restore (unchanged)

**Location**: `transaction.rs:242-267` (Staged branch, lock entry present)

### Defect 2: Edit journals get destructive recovery semantics
**File:line**: `transaction.rs:93-109, 290-328; install.rs:1348, 1753, 3557, 3686`

**Issue**: The edit flow writes `removal_kind: Staged` and NEVER updates it to `Committed`, but recovery inferred "committed removal" purely from lock-entry absence. Crash window "lock entry removed, new content not yet committed" → recovery deletes the backup (the user's OLD content) and the partial new root: the user loses the integration they were editing. The new test `edit_crash_after_lock_removal_completes_on_recovery` enshrined this destructive behavior—it was wrong.

**Fix**:
1. Added explicit intent to `RemovalJournal`: `#[serde(default)] pub intent: RemovalIntent` with `#[derive(...)] pub enum RemovalIntent { Remove, Edit }` and `#[serde(default)]` on the enum too (old journals on disk deserialize as `Remove`; old binaries reading new journals ignore the unknown field—no `deny_unknown_fields`). Kept `TRANSACTION_JOURNAL_SCHEMA_VERSION` at v3 (no bump): `#[serde(default)]` on both enum and field preserves old-binary compatibility.

2. `update_custom_integration` writes `intent: Edit` (`install.rs:1753`). `uninstall` writes `intent: Remove` (`install.rs:1348`).

3. In recovery, lock-entry-absent + `intent=Edit`:
   - If `cleanup_paths[0]` (new_root) exists → edit already completed, remove backup and journal
   - If new_root missing AND staged_path exists → RESTORE old content to new_root, re-insert `journal.removed_entry` into lock, save lock, remove journal
   - If both missing → nothing to restore, remove journal
   (`transaction.rs:290-328`)

4. Lock-entry-absent + `intent=Remove` keeps today's Committed-branch cleanup semantics (delete staged + cleanup_paths).

5. Rewrote `edit_crash_after_lock_removal_completes_on_recovery` to assert RESTORE: old lock entry back (same `updated_at`), old `provider.sh` content restored at root, backup gone (`install.rs:3557`).

6. Added test `edit_crash_after_new_content_written_keeps_new_content`: edit journal + lock entry absent + new root already exists (journal removal crashed after successful edit) → recovery keeps new content, removes backup and journal (`install.rs:3686`).

**Location**: `transaction.rs:93-109` (new types), `transaction.rs:290-328` (recovery logic), `install.rs:1348` (uninstall intent), `install.rs:1753` (edit intent), `install.rs:3557` (rewritten test), `install.rs:3686` (new test)

### Defect 3: Staged branch still drops journal on failed restore
**File:line**: `transaction.rs:276-284`

**Issue**: Pre-existing bug (same class as R3 in slice 2): In the Staged branch, `let _ = std::fs::rename(staged, &original)` ignores failure, then `remove_journal` runs unconditionally. If the rename fails (target occupied by an unexpected dir, cross-device link, permissions) the residue loses its recovery path.

**Fix**: Check rename result; if it fails and `staged` exists and `original` does not exist → `tracing::warn` + KEEP the journal for next startup (`continue` without calling `remove_journal`). Remove the journal only when:
- Restore succeeded (rename returned Ok)
- OR `staged` is gone (nothing to restore)
- OR `original` already exists (nothing to do)

**Location**: `transaction.rs:276-284` (Staged branch error handling)

### Verification After Fixes
All commands run from repo root:

```
cd src-tauri && cargo check       # ✅ success
cd src-tauri && cargo test        # ✅ 354 passed (+1 from 353), 1 ignored
cd .. && npx --legacy-peer-deps tsc --noEmit  # ✅ no errors
npm run build                     # ✅ success
node --experimental-strip-types --test tests/*.test.ts  # ✅ 76 passed
```

**Test count**: 354 passed (+4 from baseline floor of 350: +3 from original slice 3 work, +1 from review fix round)

**Changes preserved**: Phase 3 slice 3 design report (`docs/phase-reports/phase3-slice3-edit-recovery-and-single-state-design.md`) kept intact per review constraints.
